  ...
  // If it's not cached, retrieve it from the database and cache it
    ...
    try {
      const result = await client.query({
        ...
        text: 'select count(*) OVER() AS full_count, u.id, u.username, u.url, u.joined, u.last_activity, g.name, g.url from users u join user_groups g on u.group_id = g.id where u.group_id <> 2 order by ' + orderSort + ' limit $1 offset $2;',
  ...
  // If it's not cached, retrieve it from the database and cache it
    ...
    try {
      const result = await client.query({
        ...
        text: 'select count(*) OVER() AS full_count, u.id, u.username, u.url, u.joined, u.last_activity, g.name, g.url as group_url from users u join user_groups g on u.group_id = g.id where g.id = $1 order by ' + orderSort + ' limit $2 offset $3;',
...
export const groups = async () => {
  ...
  // If it's not cached, retrieve it from the database and cache it
    ...
    try {
      const result = await client.query({
        name: 'members_groups',
        text: 'select * from user_groups g where g.id > 2 order by g.id asc'
      })
    ...
    try {
      ...
      if ( args.groupID ) {
        result = await client.query({
          ...
          text: 'select count(*) OVER() AS full_count, u.id, u.username, u.url, u.joined, u.last_activity, g.name, g.url as group_url from users u join user_groups g on u.group_id = g.id and u.username ilike \'%\' || $2 || \'%\' where u.group_id = $1 order by ' + orderSort + ' limit $3 offset $4;',
          values: [ args.groupID, decodeURI(args.term), end - start, start ]
        })
      } else {
        result = await client.query({
          name: 'members_search_' + orderSort.replace(' ', '_'),
          text: 'select count(*) OVER() AS full_count, u.id, u.username, u.url, u.joined, u.last_activity, g.name, g.url as group_url from users u join user_groups g on u.group_id = g.id and u.username ilike \'%\' || $1 || \'%\' order by ' + orderSort + ' limit $2 offset $3;',
