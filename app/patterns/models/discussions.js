  ...
  // If it's not cached, retrieve it from the database and cache it
    ...
    try {
      const result = await client.query({
        ...
        text: 'select c.id as category_id, c.sort as "categorySort", c.title as "categoryTitle", c.description as "categoryDescription", d.id as discussion_id, d.sort as "discussionSort", d.title as "discussionTitle", d.url as "discussionUrl", d.description as "discussionDescription", ( select count(*) from topics where discussion_id = d.id and draft = false and private = false ) as topics, p.user_id as "lastPostAuthorID", p.created as "lastPostCreated", u.group_id as "lastPostAuthorGroupID", u.username as "lastPostAuthor", u.url as "lastPostAuthorUrl" from "categories" c join "discussions" d on c.id = d.category_id join discussion_permissions dp on d.id = dp.discussion_id and dp.group_id = $1 and dp.read = true left join "posts" p on p.id = d.last_post_id left join "users" u on p.user_id = u.id order by c.sort asc, d.sort asc;',
        values: [ groupID ]
      })
  ...
  // If it's not cached, retrieve it from the database and cache it
    ...
    try {
      const result = await client.query({
        ...
        text: 'select c.id as category_id, c.sort as "categorySort", c.title as "categoryTitle", c.description as "categoryDescription", d.id as discussion_id, d.sort as "discussionSort", d.title as "discussionTitle", d.url as "discussionUrl", d.description as "discussionDescription", d.topics, d.posts, p.user_id as "lastPostAuthorID", p.created as "lastPostCreated", u.group_id as "lastPostAuthorGroupID", u.username as "lastPostAuthor", u.url as "lastPostAuthorUrl" from "categories" c join "discussions" d on c.id = d.category_id join discussion_permissions dp on d.id = dp.discussion_id and dp.group_id = $1 and dp.post = true left join "posts" p on p.id = d.last_post_id left join "users" u on p.user_id = u.id order by c.sort asc, d.sort asc;',
        values: [ groupID ]
      })
