  ...
  if ( args.topicID.length === 1 ) {
    ...
    try {
      ...
      const oldestPost = await client.query(
        'select id, "topicID", text, html from posts where "topicID" = any($1) order by created asc limit 1;',
        [ args.topicID ]
      )

      const subscribers = await client.query(
        'select distinct "userID" from "topicSubscriptions" where "topicID" = any($1);',
        [ args.topicID ]
      )

      await client.query(
        'delete from "topicSubscriptions" where "topicID" = any($1);',
        [ args.topicID ]
      )
      ...
      await Promise.all(args.topicID.map( async (mergedTopicID) => {
        if ( mergedTopicID != oldestPost.rows[0].topicID ) {
          ...
          await client.query(
            'update "posts" set "topicID" = $1 where "topicID" = $2 and id <> ( select id from posts where "topicID" = $2 order by created asc limit 1 );',
            [ oldestPost.rows[0].topicID, mergedTopicID ]
          )
  
          let firstPost = await client.query(
            'select u.username, p.id, p.text, p.html from users u join posts p on u.id = p."userID" where p.id = ( select id from posts where "topicID" = $1 order by created asc limit 1 )',
            [ mergedTopicID ]
          )
          ...
          await client.query(
            'delete from "topicViews" where "topicID" = $1;',
            [ mergedTopicID ]
          )
        }
      }))
      ...
      await client.query(
        'update posts set "text" = $1, "html" = $2 where id = ( select id from posts where "topicID" = $3 order by created asc limit 1 );',
