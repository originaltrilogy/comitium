...
app.listen('waterfall', {
  methodGroup: function (emitter) {
    ...
    pg.connect(connectionString, function (err, client, done) {
      if ( err ) {
        ...
        client.query(
          'select "id", "url" from users order by "url" asc;',
          function (err, result) {
            done();
            if ( err ) {
              emitter.emit('error', err);
                ...
                methodGroup['row' + item.id] = function (emitter) {
                  pg.connect(connectionString, function (err, client, done) {
                    if ( err ) {
                      ...
                      client.query(
                        'select "id", "url" from users where "url" = $1 order by "joinDate" asc;',
                        [ item.url ],
                        function (err, result) {
                          if ( err ) {
                            done();
                            ...
                            if ( result.rows.length > 1 && result.rows[0].id !== item.id ) {
                                ...
                                client.query(
                                  'update users set "url" = "url" || \'-\' || "id" where "id" = $1;',
                                  [ item.id ],
                                  function (err, result) {
                                    done();
                                    if ( err ) {
  ...
  if ( output.listen.success ) {
    console.log('user url dedupe done');
  } else {
